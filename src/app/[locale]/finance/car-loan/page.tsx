...
export default function CarLoanCalculator() {
  const t = useTranslations('finance.carLoan');
  ...
  const calculate = () => {
    ...
    if (isNaN(price) || isNaN(down) || isNaN(years) || isNaN(rate) || principal <= 0 || years <= 0) {
      setResult(null);
      return;
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
            <div className="lg:col-span-7 flex flex-col gap-10">
              <div className="space-y-8">
                <div className="group">
                  <label className="block text-sm font-medium text-gray-900 mb-2">{t('carPrice')}</label>
                ...
                <div className="group">
                  <label className="block text-sm font-medium text-gray-900 mb-2">{t('downPayment')}</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <span className="text-gray-500 font-light">$</span>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <label className="block text-sm font-medium text-gray-900">{tMortgage('loanTerm')}</label>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <label className="block text-sm font-medium text-gray-900">{t('apr')}</label>
                    <div className="flex items-center gap-1 border-b border-gray-200 focus-within:border-black transition-colors">
                      <input 
                        className="w-16 text-right border-none bg-transparent py-1 px-0 text-lg font-light focus:ring-0 outline-none" 
                        type="number" 
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm flex flex-col justify-between min-h-[300px]">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-4">{tMortgage('monthlyPayment')}</p>
                  ...
                  <p className="text-sm text-gray-500 mt-4 font-light">
                    {loanTerm}-year loan at {interestRate}% APR
                  </p>
                </div>
                <div className="border-t border-gray-100 pt-6 mt-8 grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">{tMortgage('loanAmount')}</p>
                    <p className="text-lg font-medium text-gray-900">
                      {formatCurrency(loanAmount > 0 ? loanAmount : 0)}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">{tMortgage('totalCost')}</p>
                    <p className="text-lg font-medium text-gray-900">
                      {result ? formatCurrency(result.totalPayment + parseFloat(downPayment)) : '-'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg p-4 bg-gray-50 border border-gray-100 flex items-start gap-3">
                ...
                <p className="text-sm text-gray-500 leading-relaxed">
                  <span className="font-semibold text-gray-900">Tip:</span> {t('tip')}
                </p>
              </div>
            </div>
          </div>
