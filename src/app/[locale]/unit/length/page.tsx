...
export default function LengthConverter() {
  const t = useTranslations('unit.length');
  const tc = useTranslations('common');
  
  const units = [
    { code: 'm', name: t('units.meter'), factor: 1 },
    { code: 'km', name: t('units.kilometer'), factor: 1000 },
    { code: 'cm', name: t('units.centimeter'), factor: 0.01 },
    { code: 'mm', name: t('units.millimeter'), factor: 0.001 },
    { code: 'mi', name: t('units.mile'), factor: 1609.344 },
    { code: 'yd', name: t('units.yard'), factor: 0.9144 },
    { code: 'ft', name: t('units.foot'), factor: 0.3048 },
    { code: 'in', name: t('units.inch'), factor: 0.0254 },
  ];
  ...
  const calculate = () => {
    ...
    const fromFactor = units.find(u => u.code === fromUnit)?.factor || 1;
    const toFactor = units.find(u => u.code === toUnit)?.factor || 1;
